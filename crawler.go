...
func validate(c crawler) error {
	xlsx := fmt.Sprintf("%s/controle-de-arquivos-%s-%s-%s.xlsx", c.output, c.court, c.year, c.month)
	file, err := excelize.OpenFile(xlsx)
	if err != nil {
		return status.NewError(status.SystemError, fmt.Errorf("erro abrindo planilha: %w", err))
	}
	rows, err := file.GetRows("Sheet1")
	if err != nil {
		return status.NewError(status.SystemError, fmt.Errorf("erro lendo planilha: %w", err))
	}
	filenames := []string{
